class RedisStore
  ...
  class << self
    ...
    def update_feed_latency(feed_id, latency, client = REDIS_CLIENT)
      client.hset("feed-latency", feed_id, latency)
    end

    def all_processed_route_latencies(client = REDIS_CLIENT)
      client.hgetall("processed-route-latency")
    end
